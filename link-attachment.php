...
try {
    ...
    $teamMsg = str_replace("{Appointment Details Link}","http://www.thomasedisonelectric.com/call-center/scheduler-confirmation?itemid=" . $_GET['appointmentdetailslink'],$teamMsg);

    $client->messages->create(array( 
        'text' => $content,
        'phones' => implode(', ', array('1'.$_GET['phone'])) // adding 1 for the US phone numbers
    ));   
